#!/usr/bin/env bash
#
# verify-browser.sh — the browser-stack sidecar.
#
# Split out of verify.sh for two reasons:
#
#   1. It is SLOW, and it is the only slow part. It launches a real Chromium on
#      the virtual display and waits for CDP to answer, which dominated verify's
#      wall time. As a separate script, verify.sh runs it CONCURRENTLY with the
#      rest of the checks instead of serialising behind it.
#
#   2. It is the only part that depends on the DEFERRED package install
#      (agent-packages.service, see startup.tf phase B). While that is still
#      running, chromium/xvfb/zram genuinely do not exist yet. That is not drift
#      to fail on — it is a fact that is not answerable yet, which is a SKIP.
#      Without this distinction every fresh build would "fail" verification for
#      four minutes.
#
# The install state comes from `systemctl is-active agent-packages`, which
# RemainAfterExit=yes turns into a usable state machine:
#   activating -> still installing (SKIP)     active   -> finished (assert away)
#   failed     -> the install broke (FAIL)    inactive -> never ran (FAIL)
#
# Output is machine-readable so verify.sh can splice it into its own tally:
#   PASS <description> / FAIL <description> / SKIP <description>
#
# Usage:
#   ./run verify-browser            standalone, human-readable
...
#
# Exit status: 0 if nothing FAILed, 1 otherwise. SKIPs are not failures.
#
set -uo pipefail
...
# One ssh session for everything, including the browser launch. Tailscale SSH in
# check mode can demand an interactive browser confirmation per session, so the
# number of sessions is kept to a minimum.
...
# the agents' real profiles. Skipped unless chromium is actually installed —
# otherwise we would burn 25s waiting for a binary that does not exist.
...
case "$PKGS" in
activating)
  # Phase B is still working. Report it once, clearly, and skip rather than
  # emitting four failures for things that are simply not installed yet.
...
# Only assert the contents once the install claims to be done; on a failed or
# missing install these would just restate the same problem four more times.
if [[ "$PKGS" == active ]]; then
